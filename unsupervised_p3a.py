...
star_results = pd.read_csv(r"S:\Data_Science\Core\FDA_submission_10_2019\06-Data\02-Preprocessed_data\2020-02-18\AOB\AOB_DQ.csv")
    ...
    for decision in ['full_decision']:
        for ageing in ['agebin', 'reference_agebin']:
            for age in list(dq_df['agebin'].unique()):
                ...
                name = f'{erp_name}_{decision}_features'
                print(ageing, age)
                if ageing == 'reference_agebin':
                    ...
                    profile = pp.ProfileReport(
                        dq_df[(dq_df[ageing] == age)][features])
