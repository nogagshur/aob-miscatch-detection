...
if drop_visit2:
    ...
    X_train = X_train.drop_duplicates('reference')
...
dq_df = dq_df[(dq_df['agebin'] == "aob_12-16F") | (dq_df['agebin'] == "aob_12-16M") |(dq_df['agebin'] == "aob_14-19F") | (dq_df['agebin'] == "aob_14-19M")]

# Create correlation matrix
corr_matrix = dq_df[features].corr().abs()
...
auto_features = [f'encoded_feat_{i}' for i in range(enc_dim)]
df = df.dropna(subset=features)
# df[auto_features] = autoencoder(df, all_data, features, enc_dim)
# all_data[auto_features] = autoencoder(all_data, all_data, features, enc_dim)
# features = auto_features
all_data['taskData._id.$oid'] = dq_df['taskData.elm_id'].values
all_data['agebin'] = dq_df['agebin'].values
lin_reg = {j: random.choices([m for m in features[:10] if m != j], k=2) for j in features}
agebins_df = []
for age in df['agebin'].unique():
    agedf = df[df['agebin'] == age]
    agedf = make_unsup(agedf, all_data[all_data['agebin'] == age], ['P3a_Delta_Novel_attr_timeMSfromTriger',
            'P3a_Delta_Novel_attr_leftRight',
            'P3a_Delta_Novel_attr_posteriorAnterior',
...
features = features+ ['osvm', 'isof', 'gmm2_1', 'gmm2_0',  'iqr',  'cooks_d0', 'cooks_d1','cooks_d2','cooks_d3', 'cooks_d4', 'cooks_d5','cooks_d6','cooks_d7']
df = df.dropna(subset=features)
...
for train_index, test_index in cv.split(df_vis1[features], df_vis1[target]):
    ...
    # # step 3 - feature selection - maybe
    ss = StandardScaler()
    Xt = ss.fit_transform(Xt)
    Xv = ss.transform(Xv)

    # step 5 fit on training data
    xgb = LogisticRegression(max_iter=500, class_weight={0:1, 1:10}, penalty='elasticnet', solver='saga', l1_ratio=0.8).fit(Xt, yt)
