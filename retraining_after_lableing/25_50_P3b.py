...
for train_index, test_index in cv.split(df[features], df['miscatch_P3b_target']):
    ...
    export_ids[target_out==1].to_csv(fr'C:\Users\nogag\aob-miscatch-detection\retraining_after_lableing\exports_p3b\pred_miscatches_25_50_cv_{i}.csv', index=False, header=['taskData.elm_id'])
    i = i+1
...
X_pred[target_out==1]['taskData.elm_id'].to_csv(fr'C:\Users\nogag\aob-miscatch-detection\retraining_after_lableing\exports_p3b\pred_miscatches_25_50_unlabeleddata.csv', index=False, header=['taskData.elm_id'])