...
predictions = cross_val_predict(clf, df[features], df["miscatch_P3a_novel"], cv=StratifiedKFold(10), fit_params={"estimator__early_stopping_rounds":15})
...
df[predictions==1]['taskData._id.$oid'].to_csv(fr'C:\Users\nogag\aob-miscatch-detection - Copy\retraining_after_lableing\exports_p3a\pred_miscatches_lower25_cv__gridsearch.csv', index=False, header=['taskData.elm_id'])
df[(predictions == 1) & (df["miscatch_P3b_target"]==0)]['taskData._id.$oid'].to_csv(
        fr'C:\Users\nogag\aob-miscatch-detection\retraining_after_lableing\exports_p3a - Copy\false positive_miscatches_lower25_cv__gridsearch.csv',
        index=False, header=['taskData.elm_id'])
df[(predictions == 0) & (df["miscatch_P3b_target"]==1)]['taskData._id.$oid'].to_csv(
        fr'C:\Users\nogag\aob-miscatch-detection\retraining_after_lableing\exports_p3a - Copy\false negative_miscatches_lower25_cv__gridsearch.csv',
        index=False, header=['taskData.elm_id'])
...
X_pred[y_pred==1]['taskData.elm_id'].to_csv(fr'C:\Users\nogag\aob-miscatch-detection\retraining_after_lableing\exports_p3a - Copy\pred_miscatches_lower25_unlabeleddata.csv', index=False, header=['taskData.elm_id'])
