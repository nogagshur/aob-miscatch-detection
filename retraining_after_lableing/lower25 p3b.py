...
grid_search = GridSearchCV(pipeline, param_grid=param_grid, scoring='f1', cv=StratifiedKFold(3))
grid_search.fit(df[features], df["miscatch_P3b_target"], estimator__early_stopping_rounds=3)
clf = grid_search.best_estimator_
predictions = cross_val_predict(clf, df[features], df["miscatch_P3b_target"], cv=StratifiedKFold(3), fit_params={"estimator__early_stopping_rounds":3})
...
df[predictions==1]['taskData._id.$oid'].to_csv(fr'C:\Users\nogag\aob-miscatch-detection\retraining_after_lableing\exports_p3b\pred_miscatches_lower25_cv__gridsearch.csv', index=False, header=['taskData.elm_id'])
df[(predictions == 1) & (df["miscatch_P3b_target"]==0)]['taskData._id.$oid'].to_csv(
        fr'C:\Users\nogag\aob-miscatch-detection\retraining_after_lableing\exports_p3b\false positive_miscatches_lower25_cv__gridsearch.csv',
        index=False, header=['taskData.elm_id'])
df[(predictions == 0) & (df["miscatch_P3b_target"]==1)]['taskData._id.$oid'].to_csv(
        fr'C:\Users\nogag\aob-miscatch-detection\retraining_after_lableing\exports_p3b\false negative_miscatches_lower25_cv__gridsearch.csv',
        index=False, header=['taskData.elm_id'])
...
X_pred[y_pred==1]['taskData.elm_id'].to_csv(fr'C:\Users\nogag\aob-miscatch-detection\retraining_after_lableing\exports_p3b\pred_miscatches_lower25_unlabeleddata.csv', index=False, header=['taskData.elm_id'])
