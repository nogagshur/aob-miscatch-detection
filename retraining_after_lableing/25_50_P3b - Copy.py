...
from retraining_after_lableing.noneparams import param_grid_non_smote
...
new_p3a = pd.read_csv(r"C:\Users\nogag\aob-miscatch-detection\retraining_after_lableing\data\complete_aob_25-50.csv")
...
for train_index, test_index in cv.split(df[features], df['miscatch_P3b_target']):
    ...
    grid_search = GridSearchCV(pipeline, param_grid=param_grid_non_smote, scoring='roc_auc', cv=StratifiedKFold(3))
    ...
    export_ids[target_out==1].to_csv(fr'C:\Users\nogag\aob-miscatch-detection\retraining_after_lableing\exports_p3b - Copy\pred_miscatches_25_50_cv_{i}.csv', index=False, header=['taskData.elm_id'])
    i = i+1
...
X_pred[target_out==1]['taskData.elm_id'].to_csv(fr'C:\Users\nogag\aob-miscatch-detection\retraining_after_lableing\exports_p3b - Copy\pred_miscatches_25_50_unlabeleddata.csv', index=False, header=['taskData.elm_id'])