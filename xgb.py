...
for train_index, test_index in cv.split(df_vis1[features+targets], df_vis1['target']):
    ...
    target_out0 = LogisticRegression(penalty='l1', solver='liblinear', class_weight={0:1, 1:7}).fit(Xt, yt[targets[0]], ).predict_proba(Xv)[:, 0]
    target_out1 = LogisticRegression(penalty='l1', solver='liblinear', class_weight={0:1, 1:7}).fit(Xt, yt[targets[1]]).predict_proba(Xv)[:, 0]
    target_out2 = LogisticRegression(penalty='l1', solver='liblinear', class_weight={0:1, 1:7}).fit(Xt, yt['target']).predict_proba(Xv)[:, 0]
...
ss = StandardScaler()
Xt = ss.fit_transform(df[features])
Xv = ss.transform(df_test[features])


df_test['target'] = df_test[targets[0]] | df_test[targets[1]]

target_out0 = LogisticRegression(penalty='l1', solver='liblinear', class_weight={0:1, 1:7}).fit(Xt, df[targets[0]]).predict_proba(Xv)[:, 0]
target_out1 = LogisticRegression(penalty='l1', solver='liblinear', class_weight={0:1, 1:7}).fit(Xt, df[targets[1]]).predict_proba(Xv)[:, 0]
target_out2 = LogisticRegression(penalty='l1', solver='liblinear', class_weight={0:1, 1:7}).fit(Xt, df['target']).predict_proba(Xv)[:, 0]
