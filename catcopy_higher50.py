...
for train_index, test_index in cv.split(df_vis1[features+targets], df_vis1['target']):
    ...
    target_out0 = CatBoostClassifier(verbose=0, class_weights=[1, 10], depth=8).fit(Xt, yt[targets[0]],  early_stopping_rounds=5).predict_proba(Xv)[:, 0]
    target_out1 = CatBoostClassifier(verbose=0, class_weights=[1, 10], depth=8).fit(Xt, yt[targets[1]],  early_stopping_rounds=5).predict_proba(Xv)[:, 0]
    target_out2 = CatBoostClassifier(verbose=0, class_weights=[1, 10], depth=8).fit(Xt, yt['target'],  early_stopping_rounds=5).predict_proba(Xv)[:, 0]
...
target_out0 = CatBoostClassifier(verbose=0, class_weights=[1, 10], depth=8).fit(df[features], df[targets[0]],  early_stopping_rounds=5).predict_proba(df_test[features])[:, 0]
target_out1 = CatBoostClassifier(verbose=0, class_weights=[1, 10], depth=8).fit(df[features], df[targets[1]], early_stopping_rounds=5).predict_proba(df_test[features])[:, 0]
target_out2 = CatBoostClassifier(verbose=0, class_weights=[1, 10],depth=8).fit(df[features], df['target'],  early_stopping_rounds=5).predict_proba(df_test[features])[:, 0]
...
target_out0 = CatBoostClassifier(verbose=0, class_weights=[1, 10], depth=8).fit(df[features], df[targets[0]], early_stopping_rounds=5).predict_proba(X_pred[features])[:, 0]
target_out1 = CatBoostClassifier(verbose=0, class_weights=[1, 10], depth=8).fit(df[features], df[targets[1]],  early_stopping_rounds=5).predict_proba(X_pred[features])[:, 0]
target_out2 = CatBoostClassifier(verbose=0, class_weights=[1, 10], depth=8).fit(df[features], df['target'],  early_stopping_rounds=5).predict_proba(X_pred[features])[:, 0]

target_out = target_out0 * target_out1 * target_out2
print(X_pred['taskData._id.$oid'])
print(X_pred[target_out<0.75]['taskData._id.$oid'])
X_pred[target_out<0.75]['taskData._id.$oid'].to_csv(r'S:\Data_Science\Core\FDA_submission_10_2019\08-Reports\STAR_reports\Labeling_project\resource_files\miscatches_detected\higher50\delegated_reports.csv')
