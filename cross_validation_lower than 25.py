...
df = df[(df['agebin'] == "aob_12-16F") | (df['agebin'] == "aob_12-16M") | (df['agebin'] == "aob_14-19F") | (df['agebin'] == "aob_14-19M") | (df['agebin'] == "aob_18-25")]
...
target_out0 = CatBoostClassifier(verbose=0, class_weights=[1, 10], depth=4, grow_policy='Depthwise').fit(df[features], df[targets[0]]).predict_proba(df_test[features])[:, 0]
target_out1 = CatBoostClassifier(verbose=0, class_weights=[1, 10], depth=4, grow_policy='Depthwise').fit(df[features], df[targets[1]]).predict_proba(df_test[features])[:, 0]
target_out2 = CatBoostClassifier(verbose=0, class_weights=[1, 10], depth=4, grow_policy='Depthwise').fit(df[features], df['target']).predict_proba(df_test[features])[:, 0]
...
target_out0 = CatBoostClassifier(verbose=0, class_weights=[1, 10], depth=4, grow_policy='Depthwise').fit(df[features], df[targets[0]]).predict(X_pred)[:, 0]
target_out1 = CatBoostClassifier(verbose=0, class_weights=[1, 10], depth=4, grow_policy='Depthwise').fit(df[features], df[targets[1]]).predict(X_pred)[:, 0]
target_out2 = CatBoostClassifier(verbose=0,class_weights=[1, 10], depth=4, grow_policy='Depthwise').fit(df[features], df['target']).predict(X_pred)[:, 0]

target_out = target_out0 * target_out1 * target_out2
print(all_data[target_out<0.85]['taskData._id.$oid'])
all_data[target_out<0.85]['taskData._id.$oid'].to_csv('delegated_reports_lower25.csv')
